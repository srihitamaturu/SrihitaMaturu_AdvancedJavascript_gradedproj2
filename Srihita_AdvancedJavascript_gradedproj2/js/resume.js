...
function renderSearchResult(searchResult) {
    ...
    displayResumeHeader(searchResult.basics.name, searchResult.basics.AppliedFor)
    displayPersonalInfo(searchResult.basics)
    displayTechnicalSkills(searchResult.skills.keywords)
    displayHobbies(searchResult.interests.hobbies)
    displayPreviousWorkDetails(searchResult.work)
    displayProjectDetails(searchResult.projects)
    displayEducation(searchResult.education)
    displayInternships(searchResult.Internship)
    displayAchievements(searchResult.achievements.Summary)
    checkAndDisableNavButtons(searchResult)
}

function displayResumeHeader(name, appliedFor) {
...
function displayPersonalInfo(basics) {
...
function displayTechnicalSkills(skills) {
...
function displayPreviousWorkDetails(companyDetails) {
...
function displayInternships(internship) {
...
function checkAndDisableNavButtons(searchResult) {
    ...
    document.getElementById('next-button').disabled = currResultIdx === allIds.length - 1;
    document.getElementById('previous-button').disabled = currResultIdx === 0;
}
    ...
    if (searchResults.length === 0) {
        listDiv.innerHTML = `
			<div style="border: 1px solid black; border-radius: 10px; padding: 20px; margin-left: 20%; margin-right: 20%">
				<img width='80' height='80' src='img/no-result-found.png' alt='No results found'/>
